...
#	for phi in $(seq 0.1 0.1 1.0); do
	for phi in 0.85; do
		...
		for n in 30; do
			echo ./SelfishDNA -o Log_stream_nomutafter80k_Phi${phi}_Non${n}_s${s} -Size $size -Diff 0.02 -Phi $phi -Jumprate 0.01 -Uptakerate 0.01 -Seed $s -BreakChance 1.00 -Cost $cost -Scale 2 -HK 10 -Noncoding $n -MaxTime $t 
       			#echo ./SelfishDNA -o Log_ext_ednamixed_Phi${phi}_Non${n}_s${s} -Size $size -Diff 0.02 -Phi $phi -Jumprate 0.01 -Uptakerate 0.01 -Seed $s -BreakChance 1.00 -Cost $cost -Scale 2 -HK 10 -Noncoding $n -MaxTime $t -MixDNA;
