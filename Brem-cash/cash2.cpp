...
NewP3 and New3 added by Hilje and Bram for images of multiple plane displays

=== */
TYPE **NewP3()
{
  TYPE **a;
  a = (TYPE **)calloc(nrow+2+(2*margin), sizeof(TYPE *));                //removed: +2*margin//
  if (a == NULL) {
...
TYPE **New3()
{
  ...
  int i,j;
  a = NewP3(); 
  a[0] = (TYPE *)calloc((nrow+2+(2*margin))*(nplane*(ncol+2)+margin*(1+nplane)),sizeof(TYPE));        //removed: +margin*(1+nplane)//+2*margin//
  if (a[0] == NULL) {
    fprintf(stderr,"New3: Error in memory allocation\n");
    exit(1);
  }
  for (i=1,j=nrow+2+(2*margin); i < j; i++)
    a[i] = a[i-1] + (nplane*(ncol + 2)+margin*(1+nplane));                    //removed: +margin*(1+nplane)//
...
void UpdOrdReset(struct updorder **updorder_pp, bool reinit) // Reinit option added by Brem to ensure the update order struct is reset every SAVE_TIME (to ensure determinism of continued simulation)
{
  ...
  if(reinit || initial_flag == 0)
    {

      if(initial_flag == 0) *updorder_pp = (struct updorder *) malloc(length*sizeof(struct updorder));
...
void MargolusDiffusion(TYPE2** world,MARGOLUS*** margolus,int phase)
{
  ...
  /* If boundary==WRAP, do Margolus diffusion in the entire
     space. If not, but the   int counterrow; int countercol; int counterabs;
   int numabs = sizeof(Vibrio[counterrow][countercol].ab)/sizeof(char);
   for (counterrow=1; counterrow <= nrow; counterrow++){
   	for (countercol=1; countercol <= ncol; countercol++){
 		for(counterabs=0; counterabs<=numabs; counterabs++) Vibrio[counterrow][countercol].ab[counterabs] = 'S';
   	}
   } phase=0, then do the diffusion in the
